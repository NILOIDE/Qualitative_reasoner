...
def determine_transitions(states, dependencies, constraints):
    for state in states:
        ...
        possible_values = {
        idx: possible_values[idx] if idx % 2 == 0 or len(possible_values[idx]) < 2 else possible_values[idx].union(
            {'0'})
            ...
            if added_possibility:
                if output != state_values:
                    ...
                    possible_traces[idx].append(["state remains unchanged--> ",
                                                 get_corresponding_state(states, output)])
            output, derivative_change_has_been_made = fix_derivative_discrepancies(State(output), dependencies,
                                                                                   state_values)
            if derivative_change_has_been_made:
                while derivative_change_has_been_made:
                    # - Set derivative to 0 if their dependencies are 0
...
def run(args):
    ...
    print("\n######## OUTWARD CONNECTIONS AND INTER-STATE TRACE ########\n")
    for s in states:
